...
function buildCss(T) { return `
  ...
  *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
  ...
  body{font-family:${T.font};background:${T.bgRoot};color:${T.textPrimary};line-height:1.5;-webkit-font-smoothing:antialiased;zoom:1.25}
  ...
  .skel-grid{display:grid;grid-template-columns:1fr 1fr;grid-template-rows:1fr 1fr;gap:24px}
  .skel-grid .skel-hero:first-child{grid-row:1/3}
  ...
  .hero-page-wrap{max-width:1200px;margin:0 auto;padding:32px 24px;padding-top:8px}
  .hero-grid{display:grid;grid-template-columns:1fr 1fr;grid-template-rows:1fr 1fr;gap:24px;min-height:calc(100vh - 220px)}
  .hero-card:first-child{grid-row:1/3}
  .hero-card{display:flex;flex-direction:column;align-items:center;justify-content:center;text-align:center;background:${T.bgSurface};border:1px solid ${T.border};border-radius:12px;padding:32px;cursor:pointer;text-decoration:none;color:inherit;box-shadow:none;transition:box-shadow 200ms ease,border-color 200ms ease,background 150ms ease}
  ...
  .hero-card:first-child .hero-icon-wrap{width:100px;height:100px}
  .hero-card:first-child .hero-icon-svg{width:44px;height:44px}
  .hero-card:first-child .hero-title{font-size:24px}
  .hero-card:first-child .hero-desc{font-size:16px}
  ...
  .hack-result-count{font-size:13px;font-weight:500;color:${T.textMuted};margin:0}
  .hack-result-count strong{font-weight:600;color:${T.textSecondary}}
  .hack-toolbar{display:flex;align-items:center;justify-content:space-between;gap:12px;flex-wrap:wrap;margin-bottom:12px}
  .hack-sort-group{display:inline-flex;align-items:center;gap:4px}
  .hack-sort-btn{display:inline-flex;align-items:center;padding:4px 10px;font-size:11px;font-weight:500;font-family:inherit;border-radius:6px;border:1px solid ${T.border};color:${T.textMuted};background:transparent;cursor:pointer;white-space:nowrap;transition:all 150ms ease}
  .hack-sort-btn:hover{border-color:${T.accent};color:${T.textPrimary}}
  .hack-sort-btn--active{background:${T.accentSoft};border-color:${T.accent};color:${T.accent}}
  ...
  .camp-pos-group{display:flex;flex-direction:column;gap:8px}
  .camp-pos-group-label{font-size:11px;font-weight:600;text-transform:uppercase;letter-spacing:0.06em;color:${T.textMuted}}
  .camp-pos-chips{display:flex;flex-wrap:wrap;gap:8px}
  ...
  @media(max-width:768px){
    ...
    .hero-page-wrap{padding:24px 16px;padding-top:6px}
    .hero-grid{grid-template-columns:1fr;grid-template-rows:auto;min-height:auto}
    .hero-card:first-child{grid-row:auto}
    .hero-card:first-child .hero-icon-wrap{width:60px;height:60px}
    .hero-card:first-child .hero-icon-svg{width:28px;height:28px}
    .hero-card:first-child .hero-title{font-size:18px}
    .hero-card:first-child .hero-desc{font-size:14px}
  ...
  .rank-header-row{display:flex;align-items:center;justify-content:space-between;gap:16px;flex-wrap:wrap;margin-bottom:12px}
  .rank-page-title{font-size:24px;font-weight:700;color:${T.textPrimary};line-height:1.25;margin:0}
  .rank-hack-filter{display:flex;flex-wrap:wrap;gap:6px;margin-bottom:20px}
    ...
    <div className="hero-page-wrap">
      ...
      {status === "loading" && (
        <div className="hero-grid">
          <div className="skel skel-hero" style={{ gridRow: "1/3" }} />
          <div className="skel skel-hero" />
          <div className="skel skel-hero" />
        </div>
      )}
      {status === "error" && (
        <div className="hero-grid" style={{ display: "flex", alignItems: "center", justifyContent: "center" }}>
...
function HackathonListPage({ onNavigate, iconSize, onIconSizeChange }) {
  ...
  const filteredHackathons = useMemo(() => {
    ...
    return hackathons
      ...
      .sort((a, b) => {
        if (sortMode === "newest") return new Date(b.period.endAt) - new Date(a.period.endAt);
        if (sortMode === "deadline") return new Date(a.period.submissionDeadlineAt) - new Date(b.period.submissionDeadlineAt);
        // default: 상태순
  ...
  }, [hackathons, activeStatusFilter, activeTagFilters, sortMode]);
    ...
    <div className="hack-page">
      ...
      {filteredHackathons.length === 0 ? (
        ...
        <div className="hack-toolbar">
          ...
          <div className="hack-sort-group" role="group" aria-label="정렬">
            {[
              { value: "default", label: "상태순" },
              { value: "newest", label: "최신순" },
              { value: "deadline", label: "마감기한순" },
            ].map(({ value, label }) => (
              <button key={value} type="button"
                className={`hack-sort-btn ${sortMode === value ? "hack-sort-btn--active" : ""}`}
...
function CampFilterBar({ hackathons, activeSlug, onSlugChange, activeOpenFilter, onOpenFilterChange, activePositions, onPositionToggle, onPositionsClear }) {
  const [drawerOpen, setDrawerOpen] = useState(false);
  const [posDrawerOpen, setPosDrawerOpen] = useState(false);
  const drawerRef = useRef(null);
  const posDrawerRef = useRef(null);
  ...
  useEffect(() => {
    if (!drawerOpen && !posDrawerOpen) return;
    function handleClick(e) {
      if (drawerOpen && drawerRef.current && !drawerRef.current.contains(e.target)) setDrawerOpen(false);
      if (posDrawerOpen && posDrawerRef.current && !posDrawerRef.current.contains(e.target)) setPosDrawerOpen(false);
    }
    function handleEsc(e) { if (e.key === "Escape") { setDrawerOpen(false); setPosDrawerOpen(false); } }
    document.addEventListener("mousedown", handleClick);
    document.addEventListener("keydown", handleEsc);
    return () => { document.removeEventListener("mousedown", handleClick); document.removeEventListener("keydown", handleEsc); };
    ...
    <div className="camp-filter">
      <div className="camp-filter-row">
        ...
        <div style={{ display: "flex", gap: 8 }}>
          {/* Hackathon drawer trigger */}
          <span ref={drawerRef} style={{ display: "inline-flex" }}>
            <button type="button"
              className={`camp-hack-trigger ${activeSlug !== "all" ? "camp-hack-trigger--has" : ""}`}
              onClick={() => { setDrawerOpen((p) => !p); setPosDrawerOpen(false); }}
              aria-expanded={drawerOpen}>
          ...
          {/* Position filter trigger */}
          <span ref={posDrawerRef} style={{ display: "inline-flex" }}>
            <button type="button"
              className={`camp-hack-trigger ${activePositions.length > 0 ? "camp-hack-trigger--has" : ""}`}
              onClick={() => { setPosDrawerOpen((p) => !p); setDrawerOpen(false); }}
              aria-expanded={posDrawerOpen}>
              {activePositions.length > 0 ? `포지션 (${activePositions.length})` : "포지션 선택"}
              <span className={`camp-hack-arrow ${posDrawerOpen ? "camp-hack-arrow--open" : ""}`}>▾</span>
            </button>
          </span>
        </div>
      </div>
      ...
      {/* Position drawer */}
      <div className={`camp-hack-drawer-wrap ${posDrawerOpen ? "camp-hack-drawer-wrap--open" : ""}`}>
        <div className="camp-hack-drawer">
          <div className="camp-hack-drawer-body" style={{ flexDirection: "column", gap: 16 }}>
            {POSITION_GROUPS.map((group) => (
              <div key={group.label} className="camp-pos-group">
                <span className="camp-pos-group-label">{group.label}</span>
                <div className="camp-pos-chips">
                  {group.positions.map((pos) => {
                    const active = activePositions.includes(pos);
                    return (
                      <button key={pos} type="button"
                        className={`camp-chip ${active ? "camp-chip-active" : ""}`}
          ...
          <div className="camp-hack-drawer-footer">
            <span className="camp-hack-count">{activePositions.length}개 선택</span>
            <button type="button" className="camp-hack-close"
              onClick={() => { onPositionsClear(); }}
              disabled={activePositions.length === 0}>
              초기화
            </button>
            <button type="button" className="camp-hack-close" onClick={() => setPosDrawerOpen(false)}>닫기</button>
          </div>
        </div>
      </div>
      ...
      {activePositions.length > 0 && (
        <div className="filter-summary">
          {activePositions.map((pos) => (
            <span key={pos} className="summary-chip">
              {pos}
              <button type="button" className="summary-chip-remove" onClick={() => onPositionToggle(pos)} aria-label={`${pos} 제거`}>×</button>
            </span>
          ))}
          <button type="button" className="summary-clear-all" onClick={onPositionsClear}>모두 초기화</button>
        </div>
      )}
    </div>
  );
...
const POSITION_GROUPS = [
  { label: "개발", positions: ["프론트엔드", "백엔드", "iOS", "Android", "DevOps"] },
  { label: "데이터/AI", positions: ["데이터 분석", "AI/ML", "데이터 엔지니어", "MLOps"] },
  { label: "디자인/기획", positions: ["디자이너", "PM", "UX 디자이너", "QA"] },
  { label: "기타", positions: ["하드웨어 엔지니어", "IoT 개발자", "스마트 컨트랙트"] },
];
...
function CampPage({ onNavigate, initialFilter }) {
  ...
  const filteredTeams = useMemo(() => {
    return teamsWithDates
      .filter((t) => {
        ...
        if (positionFilter.length > 0 && !t.lookingFor.some((p) => positionFilter.includes(p))) return false;
        return true;
      })
      .sort((a, b) => {
  ...
  }, [teamsWithDates, filterSlug, openFilter, positionFilter]);
  ...
  return (
    <div className="camp-page">
      ...
      <CampFilterBar hackathons={hackathonList} activeSlug={filterSlug} onSlugChange={setFilterSlug}
        ...
        activePositions={positionFilter} onPositionToggle={handlePositionToggle} onPositionsClear={() => setPositionFilter([])} />
...
function deriveRankings(leaderboards, period, hackathonSlug) {
  // Step 1: Flatten (filter by hackathon if specified)
  const sources = hackathonSlug === "all"
    ? Object.values(leaderboards)
    : leaderboards[hackathonSlug] ? [leaderboards[hackathonSlug]] : [];
  const allEntries = sources.flatMap((lb) => lb.entries);
...
function RankingsPage({ onNavigate }) {
  ...
  const hackathonOptions = useMemo(() => {
    ...
    return Object.keys(leaderboards).map((slug) => {
      const h = MOCK_HACKATHONS.find((m) => m.slug === slug);
      return { slug, title: h?.title || slug };
    });
  }, [leaderboards]);

  const rankings = useMemo(
    () => leaderboards ? deriveRankings(leaderboards, activePeriod, activeHackathon) : [],
    [leaderboards, activePeriod, activeHackathon]
  );
  ...
  return (
    <div className="rank-page">
      ...
      {status === "success" && (
        <div className="rank-hack-filter">
          <button type="button"
            className={`pf-btn ${activeHackathon === "all" ? "pf-btn-active" : ""}`}
            onClick={() => setActiveHackathon("all")}>
            전체 통합
          </button>
          {hackathonOptions.map(({ slug, title }) => (
            <button key={slug} type="button"
              className={`pf-btn ${activeHackathon === slug ? "pf-btn-active" : ""}`}
              onClick={() => setActiveHackathon(slug)}>
